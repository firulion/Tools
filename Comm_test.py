...
# Logging config
logging.basicConfig(filename='C:/Users/../files/log_V01.txt', format='%(asctime)s %(levelname)-8s %(message)s', level=logging.NOTSET)
...
def loadCSV():
    global csvreader
    with open('C:/Users/../files/commands_V01.csv', 'r') as file:
...
def send_command(n):
    ...
    if (aux[0:3] in csvreader[n+1].split(',')[1]) and (len(aux) == 516): # Compare response to standard ACK response frame
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if flag == 1 or count == 4:
            count = 0
            closeCONN()
            print("Restarting connection for functionality issue!\n10 minutes remaining ")
            time.sleep(300)
